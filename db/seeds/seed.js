const db = require("../connection"),
  ...
  { convertTimestampToDate, createRef, formatComments } = require("./utils"),
  seed = async (data) => {
  ...
  CREATE TABLE comments (
    ...
    await Promise.all([categoriesPromise, usersPromise]),
      (formattedReviewData = reviewData.map(convertTimestampToDate)),
      (insertReviewsQueryStr = format(
      ...
      (reviewRows = await db
        ...
        .then((result) => result.rows)),
      (reviewIdLookup = createRef(reviewRows, "title", "review_id")),
      (formattedCommentData = formatComments(commentData, reviewIdLookup)),
      (insertCommentsQueryStr = format(
